...
public class DocumentValidator
{
    ...
    public IConfigObject ConfigObjectFull { get; private set; }
    ...
    {
        using var connectionInsurance = new SqlConnection(ConfigData.LocalDatabaseConnectionString);
    ...
    {
        using var connectionPension = new SqlConnection(ConfigData.LocalDatabaseConnectionString);
    ...
    {
        using var connectionLocal = new SqlConnection(ConfigData.LocalDatabaseConnectionString);
        using var connectionEiopa = new SqlConnection(ConfigData.EiopaDatabaseConnectionString);
    ...
    private void AssignValueToFunctionTerm(RuleStructure rule, List<RuleTerm> allTerms, RuleTerm term, string filterFomula)
    {
        ...
        switch (term.FunctionType)
        {
            ...
            case FunctionTypes.SUM:
                ...
                var isOpenTableSum = IsOpenTable(ConfigObjectFull, sumTerm.TableCode);
    ...
    private int AssignValueToPlainTerm(RuleStructure rule, RuleTerm plainTerm)
    {
        ...
        var dbValue = plainTerm.TableCode == rule.ScopeTableCode
            ? GetCellValueFromOneSheetDb(ConfigObjectFull, plainTerm.TableCode, rule.SheetId, plainTerm.Row, plainTerm.Col)
            : GetCellValueFromDbNew(ConfigObjectFull, DocumentId, plainTerm.TableCode, plainTerm.Row, plainTerm.Col);
        plainTerm.AssignDbValues(dbValue);
        return 0;
    }


    public static DbValue GetCellValueFromOneSheetDb(IConfigObject configObj, string tableCode, int sheetId, string row, string col)
    {
        using var connectionPension = new SqlConnection(configObj.Data.LocalDatabaseConnectionString);
        using var connectionEiopa = new SqlConnection(configObj.Data.EiopaDatabaseConnectionString);
    ...
    public static DbValue GetCellValueFromDbNew(IConfigObject configObj, int docId, string tableCode, string row, string col)
    {
        using var connectionLocal = new SqlConnection(configObj.Data.LocalDatabaseConnectionString);
        using var connectionEiopa = new SqlConnection(configObj.Data.EiopaDatabaseConnectionString);
        //We may have two sheets with the same sheetCode in one Document due to Z dim
        //Therefore, we must use the SheetId and not just the sheetCode for these facts. (because of sheets with the same sheetcode)
        //On the other hand, if a ruleTerm refers to a fact in another sheet, we have to use the sheet Code and NOT the sheetID
    ...
    private void CreateOneDocumentRule(RuleStructure rule, ScopeDetails scopeDetails, TemplateSheetInstance sheet, string rowCol)
    {
        ...
        newRule.ConfigObject = ConfigObjectFull;
    ...
    public void UpdateTermRowCol(RuleTerm term, string scopeTableCode, ScopeRangeAxis scopeAxis, string rowCol)
    {
        ...
        {
            var isOpenTbl = IsOpenTable(ConfigObjectFull, term.TableCode);
            if (isOpenTbl)
            {
    ...
    {
        var errorCounter = 0;
        using var connenctionLocal = new SqlConnection(ConfigObjectFull.Data.LocalDatabaseConnectionString);
        //
    ...
    private TemplateSheetFact FindFactInRowOfMasterTable(string keyDim, string tableCode, string row)
    {
        ...
        using var connectionInsurance = new SqlConnection(ConfigObjectFull.Data.LocalDatabaseConnectionString);
        using var connectionEiopa = new SqlConnection(ConfigObjectFull.Data.EiopaDatabaseConnectionString);
    ...
    private static bool IsOpenTable(IConfigObject configObj, string tablecode)
    {
        using var connectionEiopa = new SqlConnection(configObj.Data.EiopaDatabaseConnectionString);
    ...
    private decimal FunctionForOpenSumNew(RuleTerm sumTerm, string filterFormula)
    {
        ...
        var isOpenTbl = IsOpenTable(ConfigObjectFull, sumTerm.TableCode);
    ...
    private string FunctionForFtdvValue(List<RuleTerm> allTerms, RuleTerm ftdvTerm)
    {
        ...
        var fValue = GetCellValueFromDbNew(ConfigObjectFull, DocumentId, term.TableCode, term.Row, keyCol);
    ...
    {
        using var connectionLocal = new SqlConnection(ConfigObjectFull.Data.LocalDatabaseConnectionString);

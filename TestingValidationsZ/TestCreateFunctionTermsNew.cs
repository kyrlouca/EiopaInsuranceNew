...
namespace TestingValidationsZ
{
    ...
    public class TestCreateFunctionTermsNew
    {
        ...
        public void ValidateCreateFunctionTerms()
        {
            ...
            var (newFormulax, newTermsx) = RuleStructure.PrepareFunctionTerms(formulax, "Z");
            newTermsx[0].FunctionType.Should().Be(FunctionTypes.SUM);


            var formula = "X0 + X1 + empty(X1)";
            var (newFormula,newTerms) = RuleStructure.PrepareFunctionTerms(formula,"Z");
            ...
            (newFormula, newTerms) = RuleStructure.PrepareFunctionTerms(formula, "Z");
